...
namespace webMusicalShop.Controllers
{
            ...
            var claims = new List<Claim>
    {
        ...
        public async Task<IActionResult> CancelOrder(int orderId)
        {
            ...
            try
            {
                var items = order.Product.Split(',', StringSplitOptions.RemoveEmptyEntries);

                foreach (var item in items)
                {
                    string name = ExtractProductName(item);
                    int quantity = ExtractProductQuantity(item);
        ...
        public async Task<IActionResult> ExportOrderReports(string period = "week")
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Range("A1:H1").Style = headerStyle;
